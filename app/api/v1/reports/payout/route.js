...
const getHandler = async (req) => {
  try {
    ...
    const [total, rows] = await Promise.all([
      prisma.payoutOrder.count({ where }),
      prisma.payoutOrder.findMany({
        where,
    ...
    return NextResponse.json(
      { message: error?.message || "failed to fetch payout report" },
      { status: 500 },
    );
  }
};

export const GET = withApiLogging("reports/payout:GET", getHandler);
