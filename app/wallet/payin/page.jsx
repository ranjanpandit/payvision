...
export default function PayinWalletPage() {
      ...
      try {
        ...
        if (!cancelled) {
          if (!res.ok) {
            setRows([]);
            setTotal(0);
            setMessage(data?.message || "Failed to load PG wallet balances.");
            return;
          }
        ...
        if (!cancelled) {
          setRows([]);
          setTotal(0);
          setMessage("Failed to load PG wallet balances.");
        }
      } finally {
  ...
  async function openViewModal(row) {
    ...
    try {
      ...
      while (nextPage <= totalPagesFromApi) {
        ...
        const res = await fetch(`/api/v1/reports/transactions?${params.toString()}`, { cache: "no-store" });
        const data = await res.json();
        if (!res.ok) {
          throw new Error(data?.message || "Failed to load payin transactions.");
        }
      ...
      setMessage(error?.message || "Failed to load payin transactions.");
    } finally {
  ...
  function openDeductModal(row) {
    ...
    setDeductForm({
      ...
      currentPayinBalance: Number(row.pgBalance || 0),
      amount: "",
      narration: "",
      transactionPin: "",
    });
  ...
  async function submitDeduct() {
    const merchantId = String(deductForm.merchantId || "").trim();
    const amount = Number(deductForm.amount);
    const narration = String(deductForm.narration || "").trim();
    const transactionPin = String(deductForm.transactionPin || "").trim();
    ...
    setMessage("");
    try {
      const res = await fetch("/api/v1/wallet/payin/deduct", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ merchantId, amount, narration, transactionPin }),
      });
      const data = await res.json();
      if (!res.ok) {
        setMessage(data?.message || "Failed to deduct payin balance.");
        return;
      }

      const nextPayin = Number(data?.record?.payinBalance || 0);
      setRows((prev) =>
        prev.map((row) =>
      ...
      setDeductModalOpen(false);
      setMessage(`Payin deducted and moved to payout for ${merchantId}.`);
    } catch {
      setMessage("Failed to deduct payin balance.");
    } finally {
      setDeductBusy(false);
    }
  }
  ...
  return (
    <Shell title="PG Wallet" breadcrumb="Home > Manage Wallet > PG Wallet">
      {role && role !== "ADMIN" ? (
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <input
              ...
              placeholder="Merchant / Company / Email"
            />
            <button
              type="button"
              onClick={() => {
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-[#0f1f3d] text-white">
              <tr>
                ...
                <th className="px-3 py-2 text-left">PG Balance</th>
              ...
              {loading ? (
                ...
                displayRows.map((row, index) => (
                  <tr key={row.merchantId} className="border-t border-slate-200">
                    ...
                    <td className="px-3 py-2">{formatMoney(row.pgBalance)}</td>
                    <td className="px-3 py-2">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => setMessage(`Credit action for ${row.merchantId} will be added next.`)}
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/40 p-4">
          <div className="flex h-[80vh] w-full max-w-6xl flex-col rounded-lg border border-slate-300 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-300 px-4 py-3">
              <h3 className="text-lg font-semibold text-slate-800">Payin Transactions - {viewMerchantId}</h3>
              <button
                type="button"
      ...
      {deductModalOpen ? (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/40 p-4">
          <div className="w-full max-w-2xl rounded-lg border border-slate-300 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-300 px-4 py-3">
              <h3 className="text-xl font-semibold text-slate-800">Deduct Amount from PG Wallet</h3>
              <button
                type="button"
            ...
            <div className="space-y-3 px-4 py-4">
              ...
              <div>
                <label className="text-sm font-semibold text-slate-800">Current PG Balance</label>
                <input
                  value={formatMoney(deductForm.currentPayinBalance)}
                  disabled
                  className="mt-1 w-full rounded-xl border border-slate-300 bg-slate-100 px-3 py-2 text-sm"
                />
              </div>
              ...
              <div>
                <label className="text-sm font-semibold text-slate-800">Deduct Amount *</label>
                <input
                  type="number"
                  min="0"
                  step="0.01"
              ...
              <div>
                ...
                <input
                  value={deductForm.narration}
                  onChange={(e) => setDeductForm((prev) => ({ ...prev, narration: e.target.value }))}
                  placeholder="Reason for transfer to payout wallet"
                  className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="password"
                  value={deductForm.transactionPin}
                  onChange={(e) => setDeductForm((prev) => ({ ...prev, transactionPin: e.target.value }))}
                  className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"
                />
              </div>
            </div>
