...
function PayoutWalletHistoryContent() {
  ...
  async function loadHistory() {
    ...
    try {
      ...
      if (!res.ok) {
        setMessage(data?.message || "Failed to load payout passbook history.");
        setRows([]);
        return;
      }
      ...
      setMessage("Failed to load payout passbook history.");
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const viewRows = useMemo(() => {
    return rows.map((r, idx) => {
      ...
      const narration = String(parsed?.narration || (actionType ? `${actionType} Wallet` : "Transaction Success")).trim();

      const baseAmount = Number(r.amount || 0);
      const charge = Number(r.charge || 0);
      const gst = Number(r.gst || 0);
      const total = Number(r.balance || baseAmount + charge + gst);
  ...
  return (
    <Shell title="Payout Passbook History" breadcrumb="Dashboard > Payout Passbook > Payout Passbook History">
      <div className="rounded-2xl border border-slate-200 bg-white p-5 space-y-4">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={loadHistory} className="rounded-xl bg-gradient-to-r from-blue-600 to-emerald-500 px-6 py-2 text-white font-medium">Search</button>
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={10} className="px-3 py-6 text-center text-slate-600">Loading payout passbook history...</td>
                </tr>
              ) : viewRows.length === 0 ? (
                <tr>
...
export default function PayoutWalletHistoryPage() {
  return (
    <Suspense fallback={<div className="p-6 text-sm text-slate-600">Loading...</div>}>
      <PayoutWalletHistoryContent />
    </Suspense>
  );
}
