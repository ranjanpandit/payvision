...
const METHODS = { GET:"bg-emerald-100 text-emerald-700", POST:"bg-blue-100 text-blue-700", PUT:"bg-amber-100 text-amber-700", DELETE:"bg-red-100 text-red-700" };
    ...
    <div className="rounded-xl border border-slate-200 overflow-hidden">
      ...
      {open && (
        <div className="border-t border-slate-100 bg-slate-50 px-4 py-4 space-y-3 text-sm">
          ...
          {body && <div><p className="text-xs font-semibold text-slate-500 mb-1">REQUEST BODY</p><pre className="bg-slate-900 text-blue-300 rounded-lg px-3 py-2 text-xs font-mono overflow-x-auto">{JSON.stringify(JSON.parse(body),null,2)}</pre></div>}
  ...
  return (
    <Shell title="API Documentation" breadcrumb="Home > API Docs">
      <div className="space-y-5">
        {/* Header */}
        <div className="rounded-2xl bg-gradient-to-r from-blue-600 to-emerald-500 p-px">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              <div className="flex-1">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  <Badge text="REST API" style="bg-blue-100 text-blue-700"/>
        ...
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search endpoints, methods, paths..."
          className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-cyan-500"/>
