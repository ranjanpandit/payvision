export function getOpenMoneyConfig() {
  ...
  const isPlaceholder = (v) => !v || v === "CHANGE_ME" || v.startsWith("CHANGE");
  if (isPlaceholder(baseUrl) || isPlaceholder(mid) || isPlaceholder(email) || isPlaceholder(secretKey)) {
    throw new Error("Payment provider not configured — update PAYVISION_OPENMONEY_* in .env");
  }
  if (!/^https?:\/\//i.test(baseUrl)) {
    throw new Error("PAYVISION_OPENMONEY_BASE_URL must start with http:// or https://");
  }

  return { baseUrl: baseUrl.replace(/\/$/, ""), mid, email, secretKey };
}

// Backward-compatible alias for older provider routes.
export function getProviderConfig() {
  return getOpenMoneyConfig();
}
